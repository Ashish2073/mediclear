...
class VertigoReportController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('consumerImage', function ($row) {
                    ...
                    $consumerImage = '<img src="' . asset('/images/' . $row->image) . '" width="100px" alt=""> ';
    ...
    public function consumertestprofile(Request $request)
    {
        ...
        $TestData = Test::where('medical_details_id', $consumer_id)->where('test_type_id', '1')->where('test_status', '1')->whereIn('features', ['bp', 'eyecheckup', 'rt', 'flatfoot', 'bppv', 'fukuda'])->pluck('data', 'features');
        ...
        $TestresultUnfitRemark = Test::where('medical_details_id', $consumer_id)->where('test_type_id', '1')->where('test_status', '1')->where('test_results', '0')->pluck('test_results_remarks','features');
